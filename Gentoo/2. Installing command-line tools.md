...
If you don't see wireless interface, and you are running an `arm`-based device with `usb` dongle for wifi, there may be problem with switching device mode. Try installing the following package:

```sh
sudo emerge --ask sys-apps/usb_modeswitch
```

Then unplug and plug the dongle back. If it does help, then you would create a custom startup script like:

```sh
sudo tee /etc/local.d/wifi.start > /dev/null << EOF
#!/bin/bash
...
Where instead of `13-1` you should provide address of your `wifi` adapter which is indentified by running:
...
Reboot after creating the script, the adapter should start working properly.

If the interface exists, but it is down, activate the interface with command (replace `wlp2s0` with the name of your adapter):
...
Add the generated `ssh` key to your `github` and `gitlab` accounts.
...
```sh
echo 'x11-drivers/nvidia-drivers kernel-open' | sudo tee /etc/portage/package.use/nvidia-drivers
```

Accept `NVIDIA-2025` license:

```sh
echo 'x11-drivers/nvidia-drivers NVIDIA-2025' | sudo tee /etc/portage/package.license/nvidia-drivers
```
...
Install the `mesa` driver which will use options configured earlier at `/etc/portage/package.use/00video-cards`:

```sh
sudo emerge --ask media-libs/mesa
```

The command will probably ask you to enable use flag `X` for `libglvnd` - apply this change by calling `etc-update`. Then the command will run successfully, it takes some time to emerge the package.
...
::: danger Adding ssh key to github

It is important to make sure that your `ssh` key is added to your `github` account before continuing. To check that, you can try cloning one of your private repos:

```sh
git clone git@github.com:zeionara/bashrc.git /tmp/bashrc; rm -rf /tmp/bashrc
```

:::

Then sign out for the changes to have an effect. When you sign back in, there is `zsh` configuration window. Feel free to select `q` to exit configuration. Clone my fork of `ohmyzsh` with some presets (you will would need to either delete or create an own fork of `/custom/bashrc`, because it is a private repo):
...
Optionally, switch to `ssh`-based url of the origin:

```sh
git remote set-url origin git@github.com:zeionara/ohmyzsh.git
```
...
To test `pylsp` language server use a dedicated project and environment depending on your GPU:

::: tabs

=== Nvidia

You can re-use the existing `test-cuda` project:
...
You need to create a new environment with `torch` and `lsp server`:

```sh
cd; mkdir test-pylsp; cd test-pylsp
python -m venv .venv
source .venv/bin/activate
pip install torch torchvision "python-lsp-server[all]"
```
...
After you're done, deactivate `test-pylsp` environment:
...
Enable `amd64` (or `arm64` if you are installing the package on `arm` device) keyword for `mihomo` package:

::: tabs

=== AMD

The command for allowing `~amd64` keyword for `mihomo` package:
...
The command for allowing `~arm64` keyword for `mihomo` package:

```sh
echo 'net-proxy/mihomo ~arm64' | sudo tee /etc/portage/package.accept_keywords/mihomo
```
...
And update capabilities of `mihomo` binary (this operation must be repeated after each `mihomo` update):
...
Download neovim config by cloning the appropriate repo:
...
```sh
git clone -b dev git@github.com:zeionara/nvim-configs.git $HOME/.config/nvim
```

=== https

```sh
git clone -b dev https://github.com/zeionara/nvim-configs.git $HOME/.config/nvim
```

:::

Then open `neovim`, on the first startup the plugins will not be available yet, but [pckr][pckr] will install them automatically:

```sh
nvim
```

For testing syntax highligting, move to the test project created earlier, depending on your GPU:

::: tabs

=== Nvidia

Move to the root folder of the `test-cuda` project:
...
Move to the root folder of the `test-pylsp` project:
...
Then activate `test-pylsp` environment:

```sh
source .venv/bin/activate
```

If you're on the `AMD` GPU, copy `python lsp` server config in the `projects` folder of `neovim` settings:

```sh
cp $HOME/.config/nvim/projects/example.lua $HOME/.config/nvim/projects/test-pylsp.lua
```

Then open `main.py` using `neovim`, it should display diagnostic messages:
...
Enable `guru` repository first (an optional step, because `keyd` is also available in `gentoo-zh` repo):
...
Enable the required keyword:

```sh
echo 'app-misc/keyd ~amd64' | sudo tee /etc/portage/package.accept_keywords/keyd
```

Then install `keyd`:

```sh
sudo emerge --ask app-misc/keyd
```

Disable default keymaps:

```sh
cat /etc/keyd/default.conf | sed -E 's/^([^#\[\*].*)/# \1/g' | sudo tee /etc/keyd/default.conf
```

Create a custom config file, for example (see [this][keyd-configuration-example] for reference):

```sh
sudo tee /etc/keyd/tartarus.conf > /dev/null <<EOF
[ids]
1532:0201

[main]
# Map W key to Super+E
w = M-e
EOF
```

It's important to pass device ids - **each device is mapped against one config file!**

Create a custom script at `/etc/init.d/keyd`:

```sh
sudo tee /etc/init.d/keyd > /dev/null <<EOF
#!/sbin/openrc-run
...
```sh
sudo chmod +x /etc/init.d/keyd
```

Start the service in current session:

```sh
sudo rc-service keyd start
```
...
After updating config files, run the following command to apply changes:
...
Set `lua` version which will be used by the client:

```sh
echo 'net-p2p/rtorrent lua_single_target_lua5-4' | sudo tee /etc/portage/package.use/rtorrent
```

Emerge the package:

```sh
sudo emerge --ask net-p2p/rtorrent
```

Run the client by invoking `rtorrent` command, then refer to the [user guide][rtorrent-user-guide] on how to use the tool.

## Installing additional python versions (optional)

For being able to manage python slots, emerge an auxiliary package:

```sh
sudo emerge --ask app-eselect/eselect-python
```

Then list installed python versions:

```sh
eselect python list
```

Now list installable python versions:

```sh
equery list -p dev-lang/python
```

Emerge the required python version:

```sh
sudo emerge --ask dev-lang/python:3.12
```

List the installed python versions again:

```sh
eselect python list
```

And configure required version as a default python version:

```sh
sudo eselect python set 3
```

Congrats! 🎉 We've just finished installing essential command-line tools. Now it's time to move forward to configuring graphical and media apps.

[pytorch-install]: https://pytorch.org/get-started/locally
[pckr]: https://github.com/lewis6991/pckr.nvim
[keyd-configuration-example]: https://www.karimarttila.fi/keyboard/2023/11/16/linux-keyboard-configuration-with-keyd.html
[rtorrent-user-guide]: https://github.com/rakshasa/rtorrent/wiki/User-Guide#adding-and-removing-torrents
