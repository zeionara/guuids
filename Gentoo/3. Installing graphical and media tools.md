...
To install `xdg-desktop-portal-hyprland`, enable the `guru` repository (if you've installed `keyd` in the previous chapter, the repo should be enabled already):
...
If you are using hyprland, there is a `hyprshot` app. To install it, first enable `~amd64` keyword for the corresponding package:
