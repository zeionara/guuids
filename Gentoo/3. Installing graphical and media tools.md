# Installing graphical and media tools

In this chapter we go through configuring graphical and media tools for enabling convenient experience on the device.

## Configure wayland compositor

Install wayland compositor of your choice to enable graphical applications. I recommend using [Hyprland][Hyprland]. To install it, enable the `hyproverlay` repository:

```sh
sudo eselect repository add hyproverlay git https://codeberg.org/hyproverlay/hyproverlay.git
sudo emaint sync -r hyproverlay
```

Enable required USE flags:

```sh
sudo tee /etc/portage/package.accept_keywords/hyprland > /dev/null <<EOF
gui-wm/hyprland ~amd64
dev-libs/hyprlang ~amd64
dev-libs/hyprgraphics ~amd64
gui-libs/aquamarine ~amd64
gui-libs/hyprutils ~amd64
gui-libs/hyprwire ~amd64
gui-libs/hyprland-guiutils ~amd64
dev-cpp/glaze ~amd64
dev-util/hyprwayland-scanner ~amd64
gui-libs/hyprtoolkit ~amd64
gui-libs/hyprcursor ~amd64
dev-libs/hyprland-protocols ~amd64
EOF
```

I also recommend enabling `X` flag for `hyprland`, which allows to run applications which require an X server:

```sh
echo "gui-wm/hyprland X" | sudo tee /etc/portage/package.use/hyprland
```

Then emerge the required packages:

```sh
sudo emerge --ask gui-wm/hyprland
```

It will likely ask you to update `/etc/portage/package.use/installkernel`. To confirm the update, run:

```sh
sudo etc-update
```

Then type:

```sh
1
1
yes
```

Then try to emerge again, this time everything should be fine.

### Update profile

Add the following lines to your `.zprofile`:

```sh
tee ~/.zprofile > /dev/null << EOF
if test -z "\${XDG_RUNTIME_DIR}"; then
    export XDG_RUNTIME_DIR=/tmp/\${UID}-runtime-dir
    if ! test -d "\${XDG_RUNTIME_DIR}"; then
        mkdir "\${XDG_RUNTIME_DIR}"
        chmod 0700 "\${XDG_RUNTIME_DIR}"
    fi
fi
EOF
```

And restart the current session. Run the following command to make sure that changes were applied:

```sh
echo $XDG_RUNTIME_DIR
```

### Configure seatd

Enable the following flags for `seatd` package:

```sh
echo "sys-auth/seatd builtin server" | sudo tee /etc/portage/package.use/seatd
```

Then reinstall `seatd` package:

```sh
sudo emerge --ask sys-auth/seatd
```

Add your user to `seat` group:

```sh
sudo gpasswd -a zeio seat
```

Then start `seatd`:

```sh
sudo rc-service seatd start
```

And make it start automatically on boot:

```sh
sudo rc-update add seatd default
```

Now you can start `hyprland` using command to verify that the basic setup is completed:

```sh
dbus-run-session Hyprland
```

Press `Super + M` to exit `hyprland`.

To configure hyprland to spawn terminal instance on `Super + Enter`, web browser on `Super + F` and quit the active window on `Super + Q`, edit '~/.config/hypr/hyprland.conf` like this:

```sh
$terminal = alacritty
$webBrowser = firefox

bind = $mainMod, Q, killactive,
bind = $mainMod, RETURN, exec, $terminal
bind = $mainMod, F, exec, $webBrowser
```

To enable multiple layouts:

```sh
input {
    kb_layout = us,ru
    kb_options = grp:win_space_toggle
}
```

To decrease repeat delay:

```sh
input {
    repeat_delay = 200
    repeat_rate = 30
}
```

To enable opacity:

```sh
decoration {
    active_opacity = 0.8
    inactive_opacity = 0.8
}
```

Optimal value of gaps size depends on the screen:

::: tabs

=== Small screens (~1366x768)

```sh
general {
    gaps_in = 5
    gaps_out = 5
}
```

:::

### Configure cursor size and theme

There are many [cursor themes][hyprcursor] discussed at the `#hyprcursor-themes` section of the [hyprland discord channel][hyprland-discord]. I recommend using [rose-pine-hyprcursor][rose-pine-hyprcursor] or converted version of [phinger cursors][phinger-cursors].

#### Configure rose-pine-hyprcursor theme

To install `rose-pine-hyprcursor` theme, you need to download the `.tar.gz` archive with icons. Use the original repo or my cloud storage:

::: tabs

== Download from the developer's repo

Run the following command:

```sh
wget -P /tmp https://github.com/ndom91/rose-pine-hyprcursor/releases/download/v0.3.2/rose-pine-cursor-hyprcursor_0.3.2.tar.gz
```

== Download from my cloud storage

Run the following command:

```sh
wget -O /tmp/rose-pine-cursor-hyprcursor_0.3.2.tar.gz https://zeio.s3.cloud.ru/shared/themes/rose-pine-hyprcursor-0.3.2.tar.gz
```

:::

Then unpack [rose-pine-hyprcursor][rose-pine-hyprcursor] theme to `~/.local/share/icons` like this:

```sh
mkdir -p $HOME/.local/share/icons/rose-pine-hyprcursor
tar -xzvf /tmp/rose-pine-cursor-hyprcursor_0.3.2.tar.gz -C $HOME/.local/share/icons/rose-pine-hyprcursor
```

Add the following content to the `~/.config/hypr/hyprland.conf`:

```sh
env = XCURSOR_SIZE,20
env = XCURSOR_THEME,rose-pine-hyprcursor
env = HYPRCURSOR_SIZE,20
env = HYPRCURSOR_THEME,rose-pine-hyprcursor
```

#### Configure phinger-hyprcursors-light theme

::: warning Limited cursor size support

`Phinger cursors` theme supports a limited set of cursor sizes - `24`, `32`, `48`, `64`, `96` and `128`. That is why I wouldn't recommend using it.

:::

To install the converted version of [phinger cursors][phinger-cursors], download the following file from my cloud storage:

```sh
wget -P /tmp https://zeio.s3.cloud.ru/shared/themes/phinger-hyprcursors-light.tar.gz
```

Create `~/.local/share/icons` folder if it does not exist:

```sh
if test ! -d $HOME/.local/share/icons; then mkdir $HOME/.local/share/icons; fi
```

Then unpack the downloaded archive to `~/.local/share/icons` like this:

```sh
tar -xzvf /tmp/phinger-hyprcursors-light.tar.gz -C $HOME/.local/share/icons
```

Add the following content to the `~/.config/hypr/hyprland.conf`:

```sh
env = XCURSOR_SIZE,24
env = XCURSOR_THEME,phinger-hyprcursors-light
env = HYPRCURSOR_SIZE,24
env = HYPRCURSOR_THEME,phinger-hyprcursors-light
```

Now, before switching to `GUI` we still need to perform a couple of additional steps.

## Configure desktop manager

::: warning An optional section

This section has educational purposes and is not recommended to follow for setting up a system for daily use, because desktop manager is not a necessary software.

:::

Install [SDDM][SDDM], one of the most lightweight options:

```sh
sudo emerge --ask x11-misc/sddm
```

Then enable wayland for `SDDM`:

```sh
sudo mkdir -p /etc/sddm.conf
sudo tee /etc/sddm.conf.d/override.conf <<EOF > /dev/null
[General]
DisplayServer=wayland
EOF
```

Add the installed display manager to `default` run level:

```sh
sudo rc-update add display-manager default
sudo rc-service display-manager start
```

Set SDDM as the desktop manager in `/etc/conf.d/display-manager`:

```sh
echo DISPLAYMANAGER="sddm" | sudo tee /etc/conf.d/display-manager
```

The reboot, on the system startup `SDDM` should open automatically.

## Configure terminal emulator

I recommend using [alacritty][alacritty], which can be installed using this command:

```sh
sudo emerge --ask x11-terms/alacritty
```

In case there are some changes to local USE flags of other packages that should be applied, use `etc-update` tool.

::: tip Listing enabled use flags for a package

To list USE flags currently enabled for a package, run the following command:

```sh
equery u x11-terms/alacritty
```

:::

After `alacritty` is installed, create config directory:

```sh
if test ! -d $HOME/.config/alacritty; then mkdir $HOME/.config/alacritty; fi
```

And make a minimal configuration file, which may look like this:

::: tabs

=== Small screens (~1366x768)

```sh
cat <<EOF > $HOME/.config/alacritty/alacritty.toml
[mouse]
bindings=[
{ mouse = "Right", action = "Copy" }
]

[window]
padding = { x = 5, y = 5 }

[font]
size = 10
EOF
```

:::

## Configure pipewire

Enable a couple of local use-flags:

```sh
echo 'media-video/pipewire sound-server pipewire-alsa pulseaudio' | sudo tee /etc/portage/package.use/pipewire
```

Add `dbus` service to run level `boot`:

```sh
sudo rc-update add dbus boot
```

And start the service in current session:

```sh
sudo rc-service dbus start
```

Since `pipewire` is currently not supported in `OpenRC`, add the following command to `~/.config/hypr/hyprland.conf` to run `pipewire` on startup:

```sh
exec-once = gentoo-pipewire-launcher restart
```

## Troubleshooting sound issues

If `pipewire` doesn't recognize sound devices, try forcing `sof` driver (must-have for laptops with `intel` CPU):

```sh
echo 'options snd-intel-dspcfg dsp_driver=3' | sudo tee /etc/modprobe.d/sof.conf
```

Install the following packages for testing sound devices through `alsa`:

```sh
sudo emerge --ask media-sound/alsa-utils
```

List devices visible to `alsa`:

```sh
aplay -l
```

If there are no devices, check the output of:

```sh
sudo cat /proc/asound/cards
```

And see `dmesg` logs:

```sh
sudo dmesg | grep sof
```

Try adding yourself to the `audio` group, then reboot:

```sh
sudo gpasswd -a zeio audio
```

Play sound with:

```sh
peaker-test -D hw:1,0 -c 2
```

If there is no sound in `webrtc` applications, try setting `micro` volume to `100%` (replace 66 with the corresponding id from `wpctl status`):

```sh
wpctl set-volume 66 1.0
```

And use the following services to debug `webrtc` issues:

1. [Service, which does not scale volume](https://webrtc.github.io/samples/src/content/getusermedia/volume/)
2. [Service, which does scale volume](https://mictests.com/)

## Configure desktop portal

Add required keywords for the package and its dependencies:

```sh
echo "gui-libs/xdg-desktop-portal-hyprland ~amd64" | sudo tee /etc/portage/package.accept_keywords/xdg-desktop-portal-hyprland
echo "dev-cpp/sdbus-c++ ~amd64" | sudo tee -a /etc/portage/package.accept_keywords/xdg-desktop-portal-hyprland
```

::: warning Long-running command

The following command might take a lot of time depending on your hardware (on 4-core `AMD A8 6410` it took 5 hours), because it will compile and install a `qt-base` dependency.

:::

Emerge the package (this wil also install `pipewire` mentioned in the previous section):

```sh
sudo emerge --ask gui-libs/xdg-desktop-portal-hyprland
```

Create the following script to start `xdg-desktop-portal-hyprland` automatically:

```sh
cat <<EOF > $HOME/.config/hypr/xdg-desktop-portal-hyprland.sh
#!/bin/bash
sleep 1
killall xdg-desktop-portal-hyprland
killall xdg-desktop-portal-gnome
killall xdg-desktop-portal-wlr
killall xdg-desktop-portal
logger 'killed all xdg-desktop'
sleep 1
/usr/libexec/xdg-desktop-portal-hyprland &
logger 'xdg-desktop-portal-hyprland started'
sleep 2
/usr/libexec/xdg-desktop-portal &
logger 'xdg-desktop-portal started'
EOF
```

Set the permissions:

```sh
chmod +x $HOME/.config/hypr/xdg-desktop-portal-hyprland.sh
```

And update `$HOME/.config/hypr/hyprland.conf` like this:

```sh
env = XDG_CURRENT_DESKTOP, Hyprland
env = XDG_SESSION_TYPE, wayland
env = XDG_SESSION_DESKTOP, Hyprland
exec-once = dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP
exec-once = ~/.config/hypr/xdg-desktop-portal-hyprland.sh
```

## Configure OBS

Enable `~amd64` keyword for `OBS`:

```sh
echo "media-video/obs-studio ~amd64" | sudo tee /etc/portage/package.accept_keywords/obs-studio
```

Enable some local USE flags:

```sh
echo "media-video/obs-studio pipewire speex vlc websocket truetype pulseaudio" | sudo tee /etc/portage/package.use/obs-studio
```

Install `OBS`:

```sh
sudo emerge --ask media-video/obs-studio
```

Add the following line to `~/.config/hypr/hyprland.conf` to enable opening `OBS` on pressing `Super + O`:

```sh
bind = $mainMod, O, exec, obs
```

### Troubleshooting

If screensharing through pipewire doesn't work on `AMD GPU`, try to switch from `radeon` to `amdgpu` driver. On boot set the options:

```sh
radeon.si_support=0 radeon.cik_support=0 amdgpu.si_support=1 amdgpu.cik_support=1
```

If it worked, then persist changes by editing `/etc/default/grub`. Then regenerate `grub` config:

```sh
grub-mkconfig -o /boot/grub/grub.cfg
```

## Configure web browser

Now we are going to install web browser. I recommend using [firefox][firefox] because it is open-sourced and has an [explicit privacy policy][firefox-privacy].
First, I recommend enabling some local USE flags:

```sh
echo 'www-client/firefox hwaccel openh264 system-pipewire pulseaudio' | sudo tee /etc/portage/package.use/firefox
```

::: warning Long-running command

The following command might take a lot of time depending on your hardware (on 4-core `AMD A8 6410` it took 5 hours), because it will compile and install a heavy package.

:::

Then run the following command to install firefox:

```sh
sudo emerge --ask www-client/firefox
```

::: tip Jinja compilation failure

If `jinja` fails to compile `nodejs`, it is probably because `make` is running too many threads. Try lowering the number assigned to options `-j` and `-l` in the `/etc/portage/make.conf`.

:::

Add the following line to `~/.config/hypr/hyprland.conf` to enable opening `Firefox` on pressing `Super + F`:

```sh
bind = $mainMod, F, exec, $webBrowser
```

## Configure telegram messenger

Emerge the required packages:

```sh
sudo emerge --ask net-im/telegram-desktop
```

It will likely ask you for local USE flag changes, use `etc-update` to apply the suggested updates.

Add the following line to `~/.config/hypr/hyprland.conf` to enable opening `telegram` on pressing `Super + T`:

```sh
bind = $mainMod, T, exec, Telegram
```

In case there are some changes to local USE flags of other packages that should be applied, use `etc-update` tool.

## Configure bluetooth

Bluetooth should be working now. If not, try to update all packages and delete unnecessary dependencies. Try to start the service:

```sh
sudo rc-service bluetooth start
```

::: tip Update and cleanup

To update all packages in your `Gentoo` OS, sync the package repository:

```sh
sudo emaint --auto sync
```

Then update packages:

```sh
sudo emerge --ask --verbose --update --deep --newuse @world
```

Then cleanup:

```sh
sudo emerge --ask --depclean
```

:::

When bluetooth is working, add the service to `default` runlevel:

```sh
sudo rc-update add bluetooth default
```

### Unblocking bluetooth controller at startup

To check if `bluetooth` controller is blocked, run:

```sh
sudo rfkill list
```

Proceed if it shows:

```sh
Soft blocked: yes
```

#### Option 1. Create startup hook for bluetooth service

Edit `/etc/init.d/bluetooth`, insert the following lines:

```sh
start_pre() {
  rfkill unblock bluetooth
}
```

Then reboot.

#### Option 2. Create script for local service

Create script for unblocking `bluetooth` controller on startup by running the following command:

```sh
sudo tee /etc/local.d/bluetooth-unblock.start > /dev/null << EOF
#!/bin/sh
rfkill unblock bluetooth
EOF
```

Make the script executable:

```sh
sudo chmod +x /etc/local.d/bluetooth-unblock.start
```

Add `local` service to `default` runlevel if not done yet (this is usually done automatically):

```sh
sudo rc-update add local default
```

## Configure screenshoting tool

If you are using hyprland, there is a `hyprshot` app. To install it, first add `wayland-desktop` overlay:

```sh
sudo eselect repository enable wayland-desktop
sudo emaint sync --repo wayland-desktop
```

Then enable `~amd64` keyword for the corresponding package:

```sh
echo "gui-apps/hyprshot ~amd64" | sudo tee /etc/portage/package.accept_keywords/hyprshot
```

Then emerge the app:

```sh
sudo emerge --ask gui-apps/hyprshot
```

Configure binding in `~/.config/hypr/hyprland.conf`:

```sh
bind = CTRL SHIFT, S, exec, hyprshot -m region --clipboard-only -s
```

## Install file manager

I recommend using [sunflower][sunflower] file manager because it is lightweight and highly customizable. Emerge the app by running the following command:

```sh
sudo emerge --ask x11-misc/sunflower
```

Configure binding in `~/.config/hypr/hyprland.conf`:

```sh
$fileManager = sunflower
bind = $mainMod, E, exec, $fileManager
```

## Install media player

To install `ffmpeg` run the following command:

```sh
sudo emerge --ask media-video/ffmpeg
```

As a media player, I recommend using `vlc` because it supports a plenty of formats by default. Of all available [USE flags][vlc-wiki] enable at least `truetype` which is required to display subtitles:

```sh
echo 'media-video/vlc truetype' | sudo tee /etc/portage/package.use/vlc
```

Then emerge the required packages by running:

```sh
sudo emerge --ask media-video/vlc
```

## Configure missing icons

You've probably noticed that some advanced icons are missing from the default font. Let's add them to your system. To get your default font, run the following command:

```sh
fc-match
```
Which outputs something like:

```sh
LiberationSans-Regular.ttf: "Liberation Sans" "Regular"
```
Find your font on the [nerdfonts releases page][nerd-fonts] and download it:

```sh
wget -P /tmp https://github.com/ryanoasis/nerd-fonts/releases/download/v3.4.0/LiberationMono.tar.xz 
```

Unpack it to a temporary directory:

```sh
mkdir /tmp/fonts
tar -xJvf /tmp/LiberationMono.tar.xz -C /tmp/fonts
```

Now create directory for installing fonts:

```sh
sudo mkdir -p /usr/local/share/fonts
```

And move downloaded font there:

```sh
sudo mv /tmp/fonts/*.ttf /usr/local/share/fonts
```

Refresh fonts cache:

```sh
fc-cache -fv
```

Which likely won't work, in this case press `Super + M` to destroy `hyprland` session, and start a new session. Now the changes should be applied.

## Configure hibernation

Enable required `USE` flags:

```sh
sudo tee > /dev/null /etc/portage/package.use/suspend << EOF
required by sys-power/suspend-1.0_p20200924::gentoo[crypt]
# required by sys-power/suspend (argument)
>=dev-libs/libgcrypt-1.11.2-r1 static-libs
# required by sys-power/suspend-1.0_p20200924::gentoo[crypt]
# required by sys-power/suspend (argument)
>=dev-libs/libgpg-error-1.58 static-libs
# required by sys-power/suspend-1.0_p20200924::gentoo[lzo]
# required by sys-power/suspend (argument)
>=dev-libs/lzo-2.10 static-libs
EOF
```

Install the following package:

```sh
sudo emerge --ask sys-power/suspend
```

To suspend, run command:

```sh
s2ram
```

If `Nvidia GPU` is failing to suspend, which can be checked using `dmesg`:

```sh
sudo dmesg -H | tail -n 50
```

look for lines which look like:

```sh{4}
NVRM: GPU 0000:01:00.0: PreserveVideoMemoryAllocations module parameter is set. System Power Management attempted without driver procfs suspend interface. Please refer to the 'Configuring Power Management Support' section in the driver README.
nvidia 0000:01:00.0: PM: pci_pm_suspend(): nvidia_isr_kthread_bh [nvidia] returns -5
nvidia 0000:01:00.0: PM: dpm_run_callback(): pci_pm_suspend returns -5
nvidia 0000:01:00.0: PM: failed to suspend async: error -5
```

disable `NVreg_PreserveVideoMemoryAllocations=0` in `/etc/modprobe.d/nvidia.conf`:

```sh{2}
options nvidia \
  NVreg_PreserveVideoMemoryAllocations=0 \
    NVreg_TemporaryFilePath=/var/tmp
```

[Hyprland]: https://hypr.land
[hyprland-discord]: https://discord.com/invite/hQ9XvMUjjr&ved=2ahUKEwjCt6XvqbiSAxWEnP0HHdduD8AQFnoECAwQAQ&usg=AOvVaw22E8GwZpX7TAr8rXz10fKj
[hyprcursor]: https://wiki.hypr.land/Hypr-Ecosystem/hyprcursor
[rose-pine-hyprcursor]: https://github.com/ndom91/rose-pine-hyprcursor
[phinger-cursors]: https://github.com/phisch/phinger-cursors
[SDDM]: https://github.com/sddm/sddm
[alacritty]: https://github.com/alacritty/alacritty
[firefox]: https://www.firefox.com
[firefox-privacy]: https://www.mozilla.org/privacy/firefox
[sunflower]: https://sunflower-fm.org
[vlc-wiki]: https://wiki.gentoo.org/wiki/VLC
[nerd-fonts]: https://github.com/ryanoasis/nerd-fonts/releases
