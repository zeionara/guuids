...
The `nmtui` tool is missing in `arm` images, so you will need a working Ethernet connection in this setup.
...
To be extra-careful, create a new user and create their `home` directory, which will be useful even on this stage, then set password:
...
To wipe all existing data on the disk:

1. Either press `g` which will instantly overwrite current partition layout with a new GPT disklabel;
2. Either press `d` to remove partitions one by one to keep existing disklabel.

These actions lead to **irreversible data loss** and have to be made with caution.

:::

To create a new partition:

1. Type `n`, leave `partition number` default (`5` in my case).
2. First sector - default value (`268877824` in my case).
3. Also default value for the last sector (`1875384319` in my case).
4. Press `p` again to check that everything is correct.
5. Press `w` to write changes.

If I run `lsblk` now, I get the following output (the new partition is highlighted):
...
To create a `UEFI` partition layout from scratch (for example, installing without dual-booting with windows), follow these instructions:

1. Create a new GPT table - type `g`;
2. Create a new `EFI` partition - type `n`, then leave first block default, then type `+512M` to allocate `512` Mbytes for this partition, then press `t` to set type for the newly created partition, and set `1` as a type id;
3. Create a new `Linux filesystem` partition - type `n`, then leave first block default, then type desired size of the partition.
4. Create as many partitions as you'd like, then type `w` to apply changes.
5. Format `EFI` partition using command like:

```sh
mkfs.vfat -F 32 /dev/sda1
```

where `/dev/sda1` is the partition device name.

:::

Create filesystem on the newly created partition (change `/dev/sda5` to your partition label):

```sh
sudo mkfs.ext4 /dev/sda5
```

Now we need to mount the newly created partition (change `/dev/sda5` to your partition label):
...
Change the root directory permissions (if you are running remote installation through non-root user):
...
::: tip Correcting clock skew on arm images

Before proceeding installation on `arm` devices, run the following command to synchronise date and time, because `arm` images are coming with an enormous clock skew:
...
Configure global `USE` flags in `/mnt/gentoo/etc/portage/make.conf`:
...
USE="-gtk -gnome dist-kernel dbus alsa wayland bluetooth opengl nvenc cuda lua v4l screencast" # [!code ++]
...
USE="-gtk -gnome dist-kernel dbus alsa wayland bluetooth opengl lua v4l screencast" # [!code ++]
...
USE="-gtk -gnome dist-kernel egl gles2 dbus alsa wayland bluetooth opengl lua v4l screencast" # [!code ++]
...
Mount `efi` partition (in my case - `/dev/sda1`, change the value if your `nfs` partition has a different label):
...
USE="-gtk -gnome dist-kernel dbus alsa wayland bluetooth opengl nvenc cuda lua v4l screencast"
...
```sh
echo '*/* VIDEO_CARDS: -* nvidia' > /etc/portage/package.use/00video-cards
```

=== Nvidia GPU + Intel Integrated Graphics (laptop)

In this configuration `intel` driver is still needed:

```sh
echo '*/* VIDEO_CARDS: -* intel nvidia' > /etc/portage/package.use/00video-cards
```

=== AMD GPU

```sh
echo '*/* VIDEO_CARDS: -* amdgpu radeonsi' > /etc/portage/package.use/00video-cards
```

=== Mali GPU (ARM)

```sh
echo '*/* VIDEO_CARDS: -* panfrost' > /etc/portage/package.use/00video-cards
```
...
Additionally, I would recommend installing `htop` now:
...
Update the `@world` set (it takes a while to complete this command, feel free to proceed with the installation steps which don't require new software):
...
echo "kernel_cmdline=\" root=$(blkid /dev/sda5 | cut -d ' ' -f2 | cut -d '"' -f2 | sed 's/\"//g') \"" > /etc/dracut.conf.d/00-installkernel.conf
```
...
To install `linux-firmware` (which implies compiling `gentoo-kernel` due to enabled `dist-kernel` flag), run the following command:

```sh
emerge --ask sys-kernel/linux-firmware
```

Then install `sof-firmware` (this package is not available on `arm` though):
...
1. `EFI` partition corresponds to `/dev/sda1`;
2. `Windows` partition is at `/dev/sda3`;
3. `Linux` partition sits at `/sda/sda5` (as in examples above).

Then by running `blkid` commands we can trace `UUIDs` of the required partitions, these `UUIDs` we will use to refer to the partitions when writing `fstab` file:
...
For convenience install `tree`, `sudo`, `eselect-repository` and `usbutils` packages:

```sh
emerge --ask app-text/tree app-admin/sudo app-eselect/eselect-repository sys-apps/usbutils
```
...
::: tip Enabling `~arm64` keyword

Installing `fcron` on `arm` requires the `~arm64` keyword to be enabled. To enable it run the following command:

```sh
if test ! -d /etc/portage/package.accept_keywords; then mkdir /etc/portage/package.accept_keywords; fi
echo 'sys-process/fcron ~arm64' | sudo tee /etc/portage/package.accept_keywords/fcron
```
...
::: tip Installing `efibootmgr`

If `/efi` directory is empty now, then you need to install `efibootmgr` for enabling `grub` to generate it (which helps, for example, to solve bootloader generation issues on `arm` devices):

```sh
sudo emerge --ask sys-boot/efibootmgr
```
...
If we reboot the system right now, grub won't have menu entries for windows, because it doesn't detect the other operating systems on the device by default. Therefore, we need to add windows to grub menu. It is done by installing the following package (which is not available on `arm`, though):
